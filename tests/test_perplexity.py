"""Tests de honest_perplexity : vraie perplexité, pas proxy."""
...
class _DummyModel(nn.Module):
    """Modèle trivial : logits = embedding linéaire."""
...
def test_perplexity_returns_float_ge_one():
    """ppl >= 1.0 (1.0 = prédiction parfaite)."""
...
def test_perplexity_uniform_init_close_to_vocab():
    """Un modèle non-entraîné (logits ~ uniforme) → ppl ≈ vocab."""
    from fractus.metrics.perplexity import honest_perplexity
    # Avec init embedding aléatoire petite, logits ~ uniforme → CE ≈ log(vocab).
    torch.manual_seed(0)
    model = _DummyModel(vocab=10, d=4)
    inp = torch.randint(0, 10, (4, 8))
    tgt = torch.randint(0, 10, (4, 8))
    ppl = honest_perplexity(model, inp, tgt)
    # ppl ≈ exp(log(10)) = 10. Tolérance large car l'init n'est pas parfaitement uniforme.
    assert 3.0 < ppl < 30.0, f"ppl attendu ~10, eu {ppl}"


def test_perplexity_perfect_model_close_to_one():
    """Un modèle qui prédit exactement → ppl ≈ 1.0."""
    from fractus.metrics.perplexity import honest_perplexity
    model = _DummyModel(vocab=5, d=8)

    # Surfit parfait : on force head pour que logits[arg] = grand.
    inp = torch.tensor([[0, 1, 2]])
    tgt = torch.tensor([[0, 1, 2]])
    with torch.no_grad():
        # Bias pour que le bon token ait un logit énorme.
        for i in range(3):
    ...
    ppl = honest_perplexity(model, inp, tgt)
    assert ppl < 1.5, f"ppl modèle parfait attendu ~1.0, eu {ppl}"


def test_perplexity_not_just_embedding_norm():
    """CRITÈRE L6 : honest_perplexity doit faire un VRAI forward + cross_entropy,
    pas un proxy basé sur la norme de l'embedding (le mensonge FNN model.rs:537)."""
    import inspect
    ...
    assert "model(input_ids)" in src or "model(" in src, "Doit faire un vrai forward"
