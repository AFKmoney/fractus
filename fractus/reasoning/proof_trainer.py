...
Le diagnostic a revele that the reward shaping of the original est already continu et
informatif, MAIS il est ecrase a 0 for the targets ±5 (error mediane 1.7 →
correctness ≈ 0 → not of signal d'learning). Le problem n'was pas
REINFORCE ni l'architecture, but the TACHE TROP DURE DES LE DEPART.
...
class CurriculumLevel:
    """Un palier curriculum."""
    target_range: float  # targets in [-range, +range]
    min_valid_rate: float  # taux of validity requis for passer au palier suivant
    max_steps: int  # number max of not d'training a this palier
...
def shaped_reward(
    ...
    Args:
        ...
        is_valid : verdict verify exact.
        base_reward_fn : ProofReward of the original (for efficiency + diversity).
        sharpness : controle the pente of the penalty.
    Returns:
        reward : float. Compose of :
            - correctness_shaped : -log(1 + sharpness·err) normalise in [0, 1].
            - efficiency (de the original) : 1/n_steps.
            - diversity (de the original) : n_unique_rules/20.
    """
    ...
    if is_valid:
        correctness_shaped = 1.0  # bonus for validity exact.

    # Composantes efficiency + diversity inchangees (de the original ProofReward).
    eff = base_reward_fn.efficiency_reward(proof)
    div = base_reward_fn.diversity_reward(proof)

    # Memes poids that the original : 0.6 correctness + 0.3 efficiency + 0.1 diversity.
...
class ProofTrainer:
    ...
    Args:
        ...
        baseline_decay : decroissance EMA of the baseline (0.95 by defaut).
    """
    ...
    def _evaluate_valid_rate(self, target_range: float, n_eval: int = 100) -> float:
        """Taux of proofs valids on n_eval targets in [-range, range]."""
        n_valid = 0
        with torch.no_grad():
    ...
    def _evaluate_median_error(self, target_range: float, n_eval: int = 100) -> float:
        """Erreur mediane on n_eval targets."""
        errs = []
        with torch.no_grad():
            for _ in range(n_eval):
                t = float(torch.empty(1).uniform_(-target_range, target_range).item())
    ...
    def train_step(self, target_range: float) -> tuple[float, bool, float]:
        """Une etape REINFORCE with baseline, on a target in [-range, range].
        ...
        # Advantage = reward - baseline (reduit the variance REINFORCE).
        advantage = reward - self.baseline
        # Mise a jour of the baseline (EMA).
    ...
    def train(self, verbose: bool = True) -> dict:
        """Entraine on all the curriculum. Retourne a dict of metriques.

        Metriques :
            initial_error : error mediane a range=5.0 before training.
            final_error   : error mediane a range=5.0 after training.
            initial_valid_rate : taux of validity a range=5.0 before.
            final_valid_rate   : taux of validity a range=5.0 after.
            levels_reached : number of paliers atteints.
        """
        # Evaluer before.
